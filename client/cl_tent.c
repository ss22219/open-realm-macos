...
#define MAX_FLOATING_TEXTS 512     /* entries; large maps can place many persistent labels */
#define FLOATING_TEXT_CAPACITY 256 /* bytes including terminator; supports localized map text */
...
static LPCFONT CL_FloatingTextFont(floatingText_t *text) {
    LPCSTR spec;
    LPCSTR comma;
    PATHSTR name;
    DWORD size = 16;
    ...
    if (!text) return NULL;
    if (text->font >= MAX_FONTSTYLES) text->font = 0;
    if (text->font && !cl.fonts[text->font]) {
        CL_RegisterConfigString(CS_FONTS + text->font);
    }
    if (text->font && cl.fonts[text->font]) return cl.fonts[text->font];

    spec = text->font < MAX_FONTSTYLES ? cl.configstrings[CS_FONTS + text->font] : NULL;
    comma = spec ? strchr(spec, ',') : NULL;
    if (comma) {
        size = (DWORD)MAX(8, atoi(comma + 1));
        if ((size_t)(comma - spec) >= sizeof(name)) return NULL;
        memcpy(name, spec, (size_t)(comma - spec));
        name[comma - spec] = '\0';
    } else {
        strlcpy(name, "Fonts\\FRIZQT__.TTF", sizeof(name));
    }
    if (!re.LoadFont) return NULL;
    if (!text->font) text->font = (DWORD)CL_FontIndex(name, size);
    if (text->font >= MAX_FONTSTYLES) return NULL;
    if (!cl.fonts[text->font]) cl.fonts[text->font] = re.LoadFont(name, size);
    return cl.fonts[text->font];
}
        ...
        case TE_FLOATING_TEXT:
            {
                ...
                DWORD packed;
                LONG font, lifetime, fade_start, age;
                ...
                text->starttime = cl.time - (age > 0 ? (DWORD)age : 0);
...
void CL_DrawTEnts(void) {
    ...
    FOR_LOOP(i, MAX_FLOATING_TEXTS) {
        ...
        LPCFONT font = CL_FloatingTextFont(text);
        if (!font || !SCR_ProjectWorldPoint(&text->origin, &screen))
            continue;
        ...
        re.DrawText(&MAKE(drawText_t,
            .font = font, .text = text->text, .rect = rect,
