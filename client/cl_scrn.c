...
static void SCR_PreloadLoadingModels(void) {
    FOR_LOOP(i, MAX_MODELS) {
        LPCSTR name = cl.configstrings[CS_MODELS + i];
        if (!name || !*name) continue;
        if (!strstr(name, "Loading") && !strstr(name, "loading") &&
            !strstr(name, "Load-") && !strstr(name, "load-")) continue;
        if (!cl.models[i] && re.LoadModel)
            cl.models[i] = re.LoadModel(name);
    }
}
...
void SCR_DrawLoadingFallback(void) {
    ...
    /* A malformed or unavailable Loading.fdf must not remove the whole loading
    ...
    if (background) {
        ...
        frame.tex.index = background;
        frame.color = COLOR32_WHITE;
        SCR_LayoutDrawPortrait(&frame, &scene);
    } else if (re.DrawFill) {
        re.DrawFill(&scene, MAKE(COLOR32, 10, 12, 20, 255));
    }

    if (re.DrawFill) {
        bar.x = scene.x + scene.w * 0.25f;
        bar.y = scene.y + scene.h * 0.82f;
        bar.w = scene.w * 0.50f;
        bar.h = scene.h * 0.018f;
        re.DrawFill(&bar, MAKE(COLOR32, 0, 0, 0, 210));
        bar.w *= cl.loading_progress;
        re.DrawFill(&bar, MAKE(COLOR32, 235, 190, 55, 255));
    }

    if (!font && re.LoadFont)
        font = re.LoadFont("Fonts\\FRIZQT__.TTF", 18);
    if (!font || !re.DrawText) return;
    slash = strrchr(map_name ? map_name : "", '\\');
    if (!slash) slash = strrchr(map_name ? map_name : "", '/');
    if (slash) map_name = slash + 1;
    if (!map_name || !*map_name) map_name = "魔兽争霸 3";
    text_rect.x = scene.x + scene.w * 0.20f;
    text_rect.y = scene.y + scene.h * 0.70f;
    text_rect.w = scene.w * 0.60f;
    text_rect.h = scene.h * 0.08f;
    re.DrawText(&MAKE(drawText_t,
        .font = font, .text = map_name, .rect = text_rect,
        .color = COLOR32_WHITE, .textWidth = text_rect.w,
        .halign = FONT_JUSTIFYCENTER, .valign = FONT_JUSTIFYTOP));
}
