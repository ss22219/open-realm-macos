...
static BOOL WriteField1(field_t const *field, BYTE *base) {
    ...
    FOR_LOOP(i, count) {
        ...
        case F_MMOVE: {
            ...
            fprintf(stderr, "WC3 SaveGame: F_MMOVE is not supported by the WASM32 save ABI\n");
            return false;
#else
        ...
        case F_CFUNCTION: {
#ifdef __EMSCRIPTEN__
            fprintf(stderr, "WC3 SaveGame: F_CFUNCTION is not supported by the WASM32 save ABI\n");
            return false;
#else
            void *func = *(void **)p;
...
static BOOL ReadField(field_t const *field, BYTE *base) {
    ...
    FOR_LOOP(i, count) {
        ...
        case F_MMOVE: {
#ifdef __EMSCRIPTEN__
            fprintf(stderr, "WC3 LoadGame: F_MMOVE is not supported by the WASM32 save ABI\n");
            return false;
#else
        ...
        case F_CFUNCTION: {
#ifdef __EMSCRIPTEN__
            fprintf(stderr, "WC3 LoadGame: F_CFUNCTION is not supported by the WASM32 save ABI\n");
            return false;
#else
            DWORD hash = *(DWORD *)((BYTE *)p + 4);
