...
DWORD IssueNeutralImmediateOrder(LPJASS j) {
    ...
    return jass_pushboolean(j, unit && unit_issueimmediateorder(unit, jass_checkstring(j, 3)));
}
DWORD IssueNeutralImmediateOrderById(LPJASS j) {
    (void)jass_checkhandle(j, 1, "player");
    LPEDICT unit = jass_checkhandle(j, 2, "unit");
    DWORD id = (DWORD)jass_checkinteger(j, 3);
    char order[5] = { 0 };
    memcpy(order, &id, 4);
    return jass_pushboolean(j, unit && unit_issueimmediateorder(unit, order));
}
DWORD IssueNeutralPointOrder(LPJASS j) {
    (void)jass_checkhandle(j, 1, "player");
    LPEDICT unit = jass_checkhandle(j, 2, "unit");
    VECTOR2 point = { jass_checknumber(j, 4), jass_checknumber(j, 5) };
    return jass_pushboolean(j, unit && G_IssueUnitPointOrder(unit, jass_checkstring(j, 3), &point, false, unit->s.player, 0));
}
DWORD IssueNeutralPointOrderById(LPJASS j) {
    ...
    memcpy(order, &id, 4);
    VECTOR2 point = { jass_checknumber(j, 4), jass_checknumber(j, 5) };
    return jass_pushboolean(j, unit && G_IssueUnitPointOrder(unit, order, &point, false, unit->s.player, 0));
}
DWORD IssueNeutralTargetOrder(LPJASS j) {
    (void)jass_checkhandle(j, 1, "player");
    LPEDICT unit = jass_checkhandle(j, 2, "unit");
    LPEDICT target = jass_checkhandle(j, 4, "widget");
    return jass_pushboolean(j, unit && target && G_IssueUnitTargetOrder(unit, jass_checkstring(j, 3), target, false, unit->s.player));
}
DWORD IssueNeutralTargetOrderById(LPJASS j) {
    ...
    char order[5] = { 0 };
    memcpy(order, &id, 4);
    return jass_pushboolean(j, unit && target && G_IssueUnitTargetOrder(unit, order, target, false, unit->s.player));
}
