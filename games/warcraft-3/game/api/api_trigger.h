...
DWORD ResetTrigger(LPJASS j) {
    ...
    if (whichTrigger) {
        ...
        whichTrigger->eval_count = 0;
        whichTrigger->exec_count = 0;
    }
    return 0;
}
...
DWORD TriggerWaitOnSleeps(LPJASS j) {
    ...
    if (whichTrigger) whichTrigger->wait_on_sleeps = flag;
    if (QuestPeonStageDebugEnabled() && TutorialFlowDebugTrigger(whichTrigger)) {
        fprintf(stderr,
...
DWORD IsTriggerWaitOnSleeps(LPJASS j) {
    ...
    return jass_pushboolean(j, whichTrigger && whichTrigger->wait_on_sleeps);
}
...
DWORD GetTriggerEvalCount(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    return jass_pushinteger(j, whichTrigger ? whichTrigger->eval_count : 0);
}
DWORD GetTriggerExecCount(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    return jass_pushinteger(j, whichTrigger ? whichTrigger->exec_count : 0);
}
...
DWORD TriggerRegisterPlayerEventLeave(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    LPPLAYER whichPlayer = jass_checkhandle(j, 2, "player");
