...
DWORD GroupEnumUnitsOfType(LPJASS j) {
    ggroup_t *whichGroup = jass_checkhandle(j, 1, "group");
    DWORD unitid = FS_SLKKey(jass_checkstring(j, 2));
    LPCJASSFUNC filter = jass_checkhandle(j, 3, "boolexpr");
    if (!G_JassGroupValid(whichGroup)) return 0;
    FOR_LOOP(i, globals.num_edicts) {
        LPEDICT ent = &globals.edicts[i];
        if (IS_UNIT(ent) && ent->class_id == unitid && jass_evaluateboolexpr(j, filter, ent))
...
DWORD GroupImmediateOrderById(LPJASS j) {
    ...
    BOOL any = false;
    memcpy(name, &order, 4);
    if (!G_JassGroupValid(whichGroup)) return jass_pushboolean(j, false);
    FOR_LOOP(i, whichGroup->num_units) if (unit_issueimmediateorder(whichGroup->units[i], name)) any = true;
    return jass_pushboolean(j, any);
}
...
DWORD GroupPointOrderById(LPJASS j) {
    ...
    memcpy(name, &order, 4);
    if (!G_JassGroupValid(whichGroup)) return jass_pushboolean(j, false);
    FOR_LOOP(i, whichGroup->num_units) if (unit_issueorder(whichGroup->units[i], name, &point)) any = true;
    return jass_pushboolean(j, any);
}
DWORD GroupPointOrderByIdLoc(LPJASS j) {
    ...
    BOOL any = false;
    memcpy(name, &order, 4);
    if (!G_JassGroupValid(whichGroup) || !point) return jass_pushboolean(j, false);
    FOR_LOOP(i, whichGroup->num_units) if (unit_issueorder(whichGroup->units[i], name, point)) any = true;
    return jass_pushboolean(j, any);
}
...
DWORD GroupTargetOrderById(LPJASS j) {
    ...
    memcpy(name, &order, 4);
    if (!G_JassGroupValid(whichGroup)) return jass_pushboolean(j, false);
    FOR_LOOP(i, whichGroup->num_units) if (unit_issuetargetorder(whichGroup->units[i], name, targetWidget)) any = true;
    return jass_pushboolean(j, any);
}
