...
DWORD StringCase(LPJASS j) {
    ...
    char *buffer = gi.MemAlloc ? gi.MemAlloc((long)len + 1) : NULL;
    if (!buffer) return jass_pushstring(j, "");
    for (size_t i = 0; i < len; i++) {
        unsigned char ch = (unsigned char)source[i];
        buffer[i] = (char)(upper ? toupper(ch) : tolower(ch));
    }
    buffer[len] = '\0';
    DWORD result = jass_pushstring(j, buffer);
    if (gi.MemFree) gi.MemFree(buffer);
    return result;
}
...
DWORD GetHandleId(LPJASS j) {
    return jass_pushinteger(j, (LONG)jass_gethandleid(j, 1));
}

/* Multiboards are kept as lightweight VM handles.  The current renderer has
 * no Warcraft scoreboard widget tree, but maps still need the complete native
 * surface for their initialization and update loops to continue running. */
typedef struct {
    LONG rows;
    LONG columns;
    BOOL displayed;
...
typedef struct {
    wc3Multiboard_t *board;
    DWORD row, column;
} wc3MultiboardItem_t;

static wc3Multiboard_t *WC3_Multiboard(LPJASS j, int index) {
    return jass_checkhandle(j, index, "multiboard");
}

static size_t WC3_MultiboardItemIndex(wc3Multiboard_t *board, DWORD row, DWORD column) {
    if (!board || row >= 16 || column >= 16) return 0;
    return MIN((size_t)255, (size_t)row * 16u + column);
}

static wc3Multiboard_t *WC3_MultiboardItemBoard(LPJASS j, int index, size_t *item_index) {
    wc3MultiboardItem_t *item = jass_checkhandle(j, index, "multiboarditem");
    if (!item || !item->board) return NULL;
    if (item_index) *item_index = WC3_MultiboardItemIndex(item->board, item->row, item->column);
    return item->board;
}

DWORD CreateMultiboard(LPJASS j) {
    wc3Multiboard_t *board = jass_newhandle(j, sizeof(*board), "multiboard");
    return board ? 1 : 0;
}
DWORD DestroyMultiboard(LPJASS j) {
    (void)jass_checkhandle(j, 1, "multiboard");
    return 0;
}
DWORD MultiboardDisplay(LPJASS j) {
    wc3Multiboard_t *board = jass_checkhandle(j, 1, "multiboard");
    if (board) board->displayed = jass_checkboolean(j, 2);
    return 0;
}
DWORD IsMultiboardDisplayed(LPJASS j) {
    wc3Multiboard_t *board = jass_checkhandle(j, 1, "multiboard");
    return jass_pushboolean(j, board && board->displayed);
}
DWORD MultiboardGetTitleText(LPJASS j) {
    wc3Multiboard_t *board = WC3_Multiboard(j, 1);
    return jass_pushstring(j, board ? board->title : "");
}
DWORD MultiboardSetTitleText(LPJASS j) {
    wc3Multiboard_t *board = WC3_Multiboard(j, 1);
    if (board) strlcpy(board->title, jass_checkstring(j, 2), sizeof(board->title));
    return 0;
}
DWORD MultiboardGetRowCount(LPJASS j) {
    wc3Multiboard_t *board = jass_checkhandle(j, 1, "multiboard");
    return jass_pushinteger(j, board ? board->rows : 0);
}
DWORD MultiboardSetRowCount(LPJASS j) {
    wc3Multiboard_t *board = jass_checkhandle(j, 1, "multiboard");
    if (board) board->rows = jass_checkinteger(j, 2);
    return 0;
}
DWORD MultiboardGetColumnCount(LPJASS j) {
    wc3Multiboard_t *board = jass_checkhandle(j, 1, "multiboard");
    return jass_pushinteger(j, board ? board->columns : 0);
}
DWORD MultiboardSetColumnCount(LPJASS j) {
    wc3Multiboard_t *board = jass_checkhandle(j, 1, "multiboard");
    if (board) board->columns = jass_checkinteger(j, 2);
    return 0;
}
DWORD MultiboardGetItem(LPJASS j) {
    wc3MultiboardItem_t *item = jass_newhandle(j, sizeof(*item), "multiboarditem");
    if (!item) return 0;
    item->board = WC3_Multiboard(j, 1);
    item->row = MAX(0, jass_checkinteger(j, 2));
    item->column = MAX(0, jass_checkinteger(j, 3));
    return 1;
}
DWORD MultiboardReleaseItem(LPJASS j) {
    (void)jass_checkhandle(j, 1, "multiboarditem");
    return 0;
}
DWORD MultiboardSetItemValue(LPJASS j) {
    size_t index;
    wc3Multiboard_t *board = WC3_MultiboardItemBoard(j, 1, &index);
    if (board) strlcpy(board->items[index].value, jass_checkstring(j, 2), sizeof(board->items[index].value));
    return 0;
}
DWORD MultiboardSetItemStyle(LPJASS j) {
    ...
    wc3Multiboard_t *board = WC3_MultiboardItemBoard(j, 1, &index);
    if (board) {
        board->items[index].show_value = jass_checkboolean(j, 2);
        board->items[index].show_icon = jass_checkboolean(j, 3);
    }
    return 0;
}
DWORD MultiboardSetItemValueColor(LPJASS j) {
    size_t index;
    wc3Multiboard_t *board = WC3_MultiboardItemBoard(j, 1, &index);
    if (board) board->items[index].value_color = MAKE(COLOR32, jass_checkinteger(j, 2),
                                                       jass_checkinteger(j, 3), jass_checkinteger(j, 4),
                                                       jass_checkinteger(j, 5));
    return 0;
}
DWORD MultiboardSetItemWidth(LPJASS j) {
    size_t index;
    wc3Multiboard_t *board = WC3_MultiboardItemBoard(j, 1, &index);
    if (board) board->items[index].width = MAX(0.0f, jass_checknumber(j, 2));
    return 0;
}
DWORD MultiboardSetItemIcon(LPJASS j) {
    size_t index;
    wc3Multiboard_t *board = WC3_MultiboardItemBoard(j, 1, &index);
    if (board) strlcpy(board->items[index].icon, jass_checkstring(j, 2), sizeof(board->items[index].icon));
    return 0;
}
DWORD MultiboardSetItemsStyle(LPJASS j) {
    wc3Multiboard_t *board = WC3_Multiboard(j, 1);
    BOOL show_value = jass_checkboolean(j, 2), show_icon = jass_checkboolean(j, 3);
    if (board) FOR_LOOP(i, 256) { board->items[i].show_value = show_value; board->items[i].show_icon = show_icon; }
    return 0;
}
DWORD MultiboardSetItemsValue(LPJASS j) {
    wc3Multiboard_t *board = WC3_Multiboard(j, 1);
    LPCSTR value = jass_checkstring(j, 2);
    if (board) FOR_LOOP(i, 256) strlcpy(board->items[i].value, value, sizeof(board->items[i].value));
    return 0;
}
DWORD MultiboardSetItemsValueColor(LPJASS j) {
    wc3Multiboard_t *board = WC3_Multiboard(j, 1);
    COLOR32 color = MAKE(COLOR32, jass_checkinteger(j, 2), jass_checkinteger(j, 3),
                         jass_checkinteger(j, 4), jass_checkinteger(j, 5));
    if (board) FOR_LOOP(i, 256) board->items[i].value_color = color;
    return 0;
}
DWORD MultiboardSetItemsWidth(LPJASS j) {
    wc3Multiboard_t *board = WC3_Multiboard(j, 1);
    FLOAT width = MAX(0.0f, jass_checknumber(j, 2));
    if (board) FOR_LOOP(i, 256) board->items[i].width = width;
    return 0;
}
DWORD MultiboardSetItemsIcon(LPJASS j) {
    wc3Multiboard_t *board = WC3_Multiboard(j, 1);
    LPCSTR icon = jass_checkstring(j, 2);
    if (board) FOR_LOOP(i, 256) strlcpy(board->items[i].icon, icon, sizeof(board->items[i].icon));
    return 0;
}
...
DWORD SetTextTagPermanentBJ(LPJASS j);
DWORD SetTextTagSuspendedBJ(LPJASS j);
DWORD SetTextTagLifespanBJ(LPJASS j);
DWORD SetTextTagFadepointBJ(LPJASS j);
DWORD SetTextTagAgeBJ(LPJASS j);

/* A compact native hashtable implementation.  Warcraft maps use these as
 * sparse (parentKey, childKey) stores, often for thousands of timer and unit
 * records.  Keeping the table in a VM-owned handle avoids imposing a fixed
 * global limit and matches JASS handle lifetime semantics. */
typedef enum {
    WC3_HT_INTEGER,
    WC3_HT_REAL,
    WC3_HT_BOOLEAN,
    WC3_HT_STRING,
    WC3_HT_HANDLE,
} wc3HashtableValueType_t;

typedef struct wc3HashtableEntry_s {
    struct wc3HashtableEntry_s *next;
    DWORD parent, child;
    wc3HashtableValueType_t type;
    union {
        LONG integer;
        FLOAT real;
        BOOL boolean;
...
#define WC3_HASHTABLE_BUCKETS 1024
typedef struct {
    wc3HashtableEntry_t *buckets[WC3_HASHTABLE_BUCKETS];
} wc3Hashtable_t;

static wc3Hashtable_t *wc3_last_hashtable;

static DWORD WC3_HashKey(DWORD parent, DWORD child) {
    DWORD h = parent * 0x9e3779b1u + child;
    h ^= h >> 16;
    return h & (WC3_HASHTABLE_BUCKETS - 1);
}

static wc3Hashtable_t *WC3_CheckHashtable(LPJASS j, int index) {
    return (wc3Hashtable_t *)jass_checkhandle(j, index, "hashtable");
}

static wc3HashtableEntry_t *WC3_FindHashtableEntry(wc3Hashtable_t *table, DWORD parent, DWORD child) {
    wc3HashtableEntry_t *entry;
    if (!table) return NULL;
    for (entry = table->buckets[WC3_HashKey(parent, child)]; entry; entry = entry->next)
        if (entry->parent == parent && entry->child == child) return entry;
    return NULL;
}

static wc3HashtableEntry_t *WC3_GetHashtableEntry(wc3Hashtable_t *table, DWORD parent, DWORD child) {
    DWORD bucket = WC3_HashKey(parent, child);
    wc3HashtableEntry_t *entry = WC3_FindHashtableEntry(table, parent, child);
    if (entry) return entry;
    entry = calloc(1, sizeof(*entry));
    if (!entry) return NULL;
    entry->parent = parent; entry->child = child;
    entry->next = table->buckets[bucket]; table->buckets[bucket] = entry;
    return entry;
}

static void WC3_ClearHashtableValue(wc3HashtableEntry_t *entry) {
    if (entry && entry->type == WC3_HT_STRING) free(entry->value.string);
    if (entry) memset(&entry->value, 0, sizeof(entry->value));
}

DWORD InitHashtable(LPJASS j) {
    wc3Hashtable_t *table = jass_newhandle(j, sizeof(*table), "hashtable");
    if (table) wc3_last_hashtable = table;
    return table ? 1 : 0;
}

DWORD InitHashtableBJ(LPJASS j) { return InitHashtable(j); }
DWORD GetLastCreatedHashtableBJ(LPJASS j) {
    return wc3_last_hashtable
        ? jass_pushlighthandle(j, wc3_last_hashtable, "hashtable")
        : jass_pushnullhandle(j, "hashtable");
}

DWORD MultiboardSetItemValueBJ(LPJASS j) {
    (void)jass_checkhandle(j, 1, "multiboard");
    (void)jass_checkinteger(j, 2);
    (void)jass_checkinteger(j, 3);
    (void)jass_checkstring(j, 4);
    return 0;
}

static DWORD WC3_SaveHashtableInteger(LPJASS j, wc3HashtableValueType_t type) {
    wc3Hashtable_t *table = WC3_CheckHashtable(j, 1);
    wc3HashtableEntry_t *entry = WC3_GetHashtableEntry(table, (DWORD)jass_checkinteger(j, 2), (DWORD)jass_checkinteger(j, 3));
    if (!entry) return 0;
    WC3_ClearHashtableValue(entry); entry->type = type;
    if (type == WC3_HT_INTEGER) entry->value.integer = jass_checkinteger(j, 4);
    else if (type == WC3_HT_BOOLEAN) entry->value.boolean = jass_checkboolean(j, 4);
    else entry->value.real = jass_checknumber(j, 4);
    return 0;
}

DWORD SaveInteger(LPJASS j) { return WC3_SaveHashtableInteger(j, WC3_HT_INTEGER); }
DWORD SaveReal(LPJASS j) { return WC3_SaveHashtableInteger(j, WC3_HT_REAL); }
DWORD SaveBoolean(LPJASS j) { return WC3_SaveHashtableInteger(j, WC3_HT_BOOLEAN); }
...
DWORD SaveStr(LPJASS j) {
    ...
    if (!entry) return 0;
    WC3_ClearHashtableValue(entry); entry->type = WC3_HT_STRING;
    entry->value.string = strdup(value ? value : "");
    return 1;
}

static DWORD WC3_LoadHashtable(LPJASS j, wc3HashtableValueType_t type, LPCSTR handle_type) {
    wc3Hashtable_t *table = WC3_CheckHashtable(j, 1);
    wc3HashtableEntry_t *entry = WC3_FindHashtableEntry(table, (DWORD)jass_checkinteger(j, 2), (DWORD)jass_checkinteger(j, 3));
    if (!entry || entry->type != type) {
        if (type == WC3_HT_REAL) return jass_pushnumber(j, 0);
        if (type == WC3_HT_BOOLEAN) return jass_pushboolean(j, false);
        if (type == WC3_HT_STRING) return jass_pushstring(j, "");
        if (type == WC3_HT_HANDLE) return jass_pushnullhandle(j, handle_type);
        return jass_pushinteger(j, 0);
    }
    if (type == WC3_HT_REAL) return jass_pushnumber(j, entry->value.real);
    if (type == WC3_HT_BOOLEAN) return jass_pushboolean(j, entry->value.boolean);
    if (type == WC3_HT_STRING) return jass_pushstring(j, entry->value.string);
    if (type == WC3_HT_HANDLE) return jass_pushlighthandle(j, entry->value.handle, handle_type);
    return jass_pushinteger(j, entry->value.integer);
}

DWORD LoadInteger(LPJASS j) { return WC3_LoadHashtable(j, WC3_HT_INTEGER, NULL); }
DWORD LoadReal(LPJASS j) { return WC3_LoadHashtable(j, WC3_HT_REAL, NULL); }
DWORD LoadBoolean(LPJASS j) { return WC3_LoadHashtable(j, WC3_HT_BOOLEAN, NULL); }
DWORD LoadStr(LPJASS j) { return WC3_LoadHashtable(j, WC3_HT_STRING, NULL); }

static DWORD WC3_SaveHashtableHandle(LPJASS j, LPCSTR type) {
    wc3Hashtable_t *table = WC3_CheckHashtable(j, 1);
    wc3HashtableEntry_t *entry = WC3_GetHashtableEntry(table, (DWORD)jass_checkinteger(j, 2), (DWORD)jass_checkinteger(j, 3));
    if (!entry) return 0;
    WC3_ClearHashtableValue(entry); entry->type = WC3_HT_HANDLE;
    entry->value.handle = jass_checkhandle(j, 4, type);
    return 0;
}

#define WC3_HT_HANDLE_FUNCS(TYPE, NAME) \
DWORD Save##NAME##Handle(LPJASS j) { return WC3_SaveHashtableHandle(j, TYPE); } \
DWORD Load##NAME##Handle(LPJASS j) { return WC3_LoadHashtable(j, WC3_HT_HANDLE, TYPE); }
WC3_HT_HANDLE_FUNCS("player", Player)
WC3_HT_HANDLE_FUNCS("unit", Unit)
WC3_HT_HANDLE_FUNCS("item", Item)
WC3_HT_HANDLE_FUNCS("effect", Effect)
WC3_HT_HANDLE_FUNCS("trigger", Trigger)
WC3_HT_HANDLE_FUNCS("timer", Timer)
WC3_HT_HANDLE_FUNCS("group", Group)
WC3_HT_HANDLE_FUNCS("location", Location)
WC3_HT_HANDLE_FUNCS("boolexpr", BooleanExpr)
WC3_HT_HANDLE_FUNCS("force", Force)
WC3_HT_HANDLE_FUNCS("lightning", Lightning)
WC3_HT_HANDLE_FUNCS("texttag", TextTag)

static DWORD WC3_HaveHashtable(LPJASS j, wc3HashtableValueType_t type) {
    wc3Hashtable_t *table = WC3_CheckHashtable(j, 1);
    wc3HashtableEntry_t *entry = WC3_FindHashtableEntry(table, (DWORD)jass_checkinteger(j, 2), (DWORD)jass_checkinteger(j, 3));
    return jass_pushboolean(j, entry && entry->type == type);
}
DWORD HaveSavedInteger(LPJASS j) { return WC3_HaveHashtable(j, WC3_HT_INTEGER); }
DWORD HaveSavedReal(LPJASS j) { return WC3_HaveHashtable(j, WC3_HT_REAL); }
DWORD HaveSavedBoolean(LPJASS j) { return WC3_HaveHashtable(j, WC3_HT_BOOLEAN); }
DWORD HaveSavedString(LPJASS j) { return WC3_HaveHashtable(j, WC3_HT_STRING); }
DWORD HaveSavedHandle(LPJASS j) { return WC3_HaveHashtable(j, WC3_HT_HANDLE); }

static DWORD WC3_RemoveHashtable(LPJASS j) {
    wc3Hashtable_t *table = WC3_CheckHashtable(j, 1);
    DWORD parent = (DWORD)jass_checkinteger(j, 2), child = (DWORD)jass_checkinteger(j, 3);
    DWORD bucket = WC3_HashKey(parent, child);
    wc3HashtableEntry_t **link = table ? &table->buckets[bucket] : NULL;
    while (link && *link) {
        if ((*link)->parent == parent && (*link)->child == child) {
            wc3HashtableEntry_t *entry = *link;
            *link = entry->next; WC3_ClearHashtableValue(entry); free(entry); break;
        }
        link = &(*link)->next;
    }
    return 0;
}
DWORD RemoveSavedInteger(LPJASS j) { return WC3_RemoveHashtable(j); }
DWORD RemoveSavedReal(LPJASS j) { return WC3_RemoveHashtable(j); }
DWORD RemoveSavedBoolean(LPJASS j) { return WC3_RemoveHashtable(j); }
DWORD RemoveSavedString(LPJASS j) { return WC3_RemoveHashtable(j); }
DWORD RemoveSavedHandle(LPJASS j) { return WC3_RemoveHashtable(j); }
DWORD FlushChildHashtable(LPJASS j) {
    wc3Hashtable_t *table = WC3_CheckHashtable(j, 1);
    DWORD parent = (DWORD)jass_checkinteger(j, 2);
    if (!table) return 0;
    for (DWORD bucket = 0; bucket < WC3_HASHTABLE_BUCKETS; bucket++) {
        wc3HashtableEntry_t **link = &table->buckets[bucket];
        while (*link) {
            wc3HashtableEntry_t *entry = *link;
            if (entry->parent == parent) {
                *link = entry->next;
                WC3_ClearHashtableValue(entry);
                free(entry);
            } else {
                link = &entry->next;
            }
...
DWORD FlushParentHashtable(LPJASS j) {
    ...
    if (!table) return 0;
    for (DWORD bucket = 0; bucket < WC3_HASHTABLE_BUCKETS; bucket++) {
        wc3HashtableEntry_t *entry = table->buckets[bucket];
        while (entry) {
            wc3HashtableEntry_t *next = entry->next;
            WC3_ClearHashtableValue(entry);
            free(entry);
            entry = next;
        }
