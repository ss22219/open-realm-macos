...
DWORD SetUnitTimeScale(LPJASS j) {
    ...
    if (whichUnit) whichUnit->animation_time_scale = MAX(0.0f, timeScale);
    return 0;
}
DWORD SetUnitBlendTime(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    FLOAT blendTime = jass_checknumber(j, 2);
    if (whichUnit) whichUnit->animation_blend_time = MAX(0.0f, blendTime);
    return 0;
}
DWORD SetUnitVertexColor(LPJASS j) {
    ...
    if (whichUnit) whichUnit->vertex_color = color;
    return 0;
}
...
DWORD SetUnitAnimationWithRarity(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    LPCSTR whichAnimation = jass_checkstring(j, 2);
    (void)jass_checkhandle(j, 3, "raritycontrol");
    if (whichUnit) G_SetUnitAnimation(whichUnit, whichAnimation);
    return 0;
}
...
DWORD UnitHasBuffBJ(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    LONG buffcode = jass_checkinteger(j, 2);
    return jass_pushboolean(j, whichUnit && G_UnitAbilityLevel(whichUnit, (DWORD)buffcode) > 0);
}
...
DWORD ReviveHeroLoc(LPJASS j) {
    ...
    if (!whichHero || !loc) return jass_pushboolean(j, false);
    G_ReviveHero(whichHero, loc->x, loc->y);
    return jass_pushboolean(j, true);
}
DWORD SetUnitExploded(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    BOOL exploded = jass_checkboolean(j, 2);
    if (whichUnit) whichUnit->exploded = exploded;
    return 0;
}
...
DWORD GetUnitPointValue(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    return jass_pushinteger(j, whichUnit && whichUnit->data.UnitData ? whichUnit->data.UnitData->points : 0);
}
DWORD GetUnitPointValueByType(LPJASS j) {
    DWORD unitType = jass_checkinteger(j, 1);
    return jass_pushinteger(j, G_UnitData(unitType)->points);
}
...
DWORD UnitUseItem(LPJASS j) {
    ...
    LPEDICT whichItem = jass_checkhandle(j, 2, "item");
    if (!whichUnit || !whichItem) return jass_pushboolean(j, false);
    FOR_LOOP(i, G_InventoryCapacity(whichUnit)) {
        if (whichUnit->inventory[i] == whichItem) {
            G_UseItem(whichUnit, (DWORD)i);
            return jass_pushboolean(j, true);
        }
    }
    return jass_pushboolean(j, false);
...
DWORD IsHeroUnitId(LPJASS j) {
    DWORD unitId = jass_checkinteger(j, 1);
    LPCSTR list = G_UnitAbil(unitId)->heroAbilList;
    return jass_pushboolean(j, list && *list);
}
...
DWORD UnitRemoveBuffs(LPJASS j) {
    ...
    if (whichUnit && (removePositive || removeNegative)) {
        FOR_LOOP(i, MAX_UNIT_STATUSES) {
            if (whichUnit->abilstatus[i].level && whichUnit->abilstatus[i].timestamp)
                memset(whichUnit->abilstatus + i, 0, sizeof(whichUnit->abilstatus[i]));
        }
    }
    return 0;
}
DWORD UnitAddSleep(LPJASS j) {
    ...
    if (whichUnit) whichUnit->sleeping = add;
    return 0;
}
DWORD UnitCanSleep(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    return jass_pushboolean(j, whichUnit && whichUnit->data.UnitData && whichUnit->data.UnitData->canSleep);
}
DWORD UnitAddSleepPerm(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    BOOL add = jass_checkboolean(j, 2);
    if (whichUnit) whichUnit->sleeping_permanent = add;
    return 0;
}
...
DWORD UnitIsSleeping(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    return jass_pushboolean(j, whichUnit && whichUnit->sleeping);
}
DWORD UnitWakeUp(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    if (whichUnit) whichUnit->sleeping = false;
    return 0;
}
DWORD UnitApplyTimedLife(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    DWORD buffId = jass_checkinteger(j, 2);
    FLOAT duration = jass_checknumber(j, 3);
    if (whichUnit && duration > 0.0f) unit_addtimedstatus(whichUnit, (LPCSTR)&buffId, 1, duration);
    return 0;
}
...
DWORD IssueImmediateOrderById(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    DWORD order = jass_checkinteger(j, 2);
    char name[5] = { 0 };
    memcpy(name, &order, 4);
    return jass_pushboolean(j, unit_issueimmediateorder(whichUnit, name));
}
...
DWORD IssuePointOrderById(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    DWORD order = jass_checkinteger(j, 2);
    VECTOR2 point = MAKE(VECTOR2, jass_checknumber(j, 3), jass_checknumber(j, 4));
    char name[5] = { 0 };
    memcpy(name, &order, 4);
    return jass_pushboolean(j, unit_issueorder(whichUnit, name, &point));
}
DWORD IssuePointOrderByIdLoc(LPJASS j) {
    LPEDICT whichUnit = jass_checkhandle(j, 1, "unit");
    DWORD order = jass_checkinteger(j, 2);
    LPCVECTOR2 point = jass_checkhandle(j, 3, "location");
    char name[5] = { 0 };
    memcpy(name, &order, 4);
    return jass_pushboolean(j, point && unit_issueorder(whichUnit, name, point));
}
...
DWORD IssueTargetOrderById(LPJASS j) {
    ...
    char name[5] = { 0 };
    memcpy(name, &order, 4);
    return jass_pushboolean(j, unit_issuetargetorder(whichUnit, name, targetWidget));
}
...
DWORD WaygateGetDestinationX(LPJASS j) {
    ...
    return jass_pushnumber(j, waygate ? waygate->waygate_destination.x : 0);
}
DWORD WaygateGetDestinationY(LPJASS j) {
    LPEDICT waygate = jass_checkhandle(j, 1, "unit");
    return jass_pushnumber(j, waygate ? waygate->waygate_destination.y : 0);
}
DWORD WaygateSetDestination(LPJASS j) {
    LPEDICT waygate = jass_checkhandle(j, 1, "unit");
    FLOAT x = jass_checknumber(j, 2), y = jass_checknumber(j, 3);
    if (waygate) waygate->waygate_destination = MAKE(VECTOR2, x, y);
    return 0;
}
DWORD WaygateActivate(LPJASS j) {
    LPEDICT waygate = jass_checkhandle(j, 1, "unit");
    BOOL activate = jass_checkboolean(j, 2);
    if (waygate) waygate->waygate_active = activate;
    return 0;
}
DWORD WaygateIsActive(LPJASS j) {
    LPEDICT waygate = jass_checkhandle(j, 1, "unit");
    return jass_pushboolean(j, waygate && waygate->waygate_active);
}
...
DWORD CreateUnitByName(LPJASS j) {
    ...
    DWORD unitid = FS_SLKKey(unitname);
    LPEDICT unit = player ? unit_createorfind(PLAYER_NUM(player), unitid, &location, face) : NULL;
    return jass_pushlighthandle(j, unit, "unit");
}
