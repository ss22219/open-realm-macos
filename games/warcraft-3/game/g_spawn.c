...
//        }
    ...
    if (!jass_dofile(level.vm, "Scripts\\common.j"))
        fprintf(stderr, "JASS: failed to load Scripts\\common.j: %s\n", jass_rterror_message(level.vm));
    if (!jass_dofile(level.vm, "Scripts\\Blizzard.j"))
        fprintf(stderr, "JASS: failed to load Scripts\\Blizzard.j: %s\n", jass_rterror_message(level.vm));
//    jass_dofilenative(level.vm, "/Users/igor/Desktop/war3map.j");
    G_DumpPrologue02BurrowHandoffSource(level.mapinfo->mapscript);
    fprintf(stderr, "JASS: compiling map script (%zu bytes)\n",
            level.mapinfo->mapscript ? strlen(level.mapinfo->mapscript) : 0u);
    if (!jass_dobuffer(level.vm, level.mapinfo->mapscript)) {
        fprintf(stderr, "JASS: map script compilation failed: %s\n",
                jass_rterror_message(level.vm));
    } else {
        fprintf(stderr, "JASS: map script compiled\n");
    }
