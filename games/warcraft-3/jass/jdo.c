...
DWORD jass_gethandleid(LPJASS j, int index) {
    LPCJASSVAR var = jass_stackvalue(j, index);
    if (!var || !var->value)
        return 0;
    if (var->ref && var->ref->id)
        return var->ref->id;

    /* Native light handles do not have VM reference records.  Derive a
     * stable non-zero id from the host pointer so hashtable keys work for
     * timers, units, groups, and other engine-owned handles as in Warcraft. */
    uintptr_t value = (uintptr_t)var->value;
    value ^= value >> 33;
    value *= (uintptr_t)0xff51afd7ed558ccdULL;
    value ^= value >> 33;
    value *= (uintptr_t)0xc4ceb9fe1a85ec53ULL;
    value ^= value >> 33;
    DWORD id = (DWORD)(value ^ (value >> 32));
    return id ? id : 1;
}
...
DWORD VM_EvalInteger(LPJASS j, LPCTOKEN token) {
    long value;
    if (token->primary && token->primary[0] == '$')
        value = strtol(token->primary + 1, NULL, 16);
    else
        value = strtol(token->primary, NULL, 10);
    return jass_pushinteger(j, (DWORD)value);
}
