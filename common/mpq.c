...
static unsigned int Mpq_PkwareRead(char *buffer, unsigned int *size, void *param) {
    mpqPkwareStream_t *stream = (mpqPkwareStream_t *)param;
    DWORD available = stream->input_size - stream->input_pos;
    if (*size > available) *size = available;
    if (*size) memcpy(buffer, stream->input + stream->input_pos, *size);
    stream->input_pos += *size;
    return *size;
}

static void Mpq_PkwareWrite(char *buffer, unsigned int *size, void *param) {
    mpqPkwareStream_t *stream = (mpqPkwareStream_t *)param;
    DWORD available = stream->output_size - stream->output_pos;
    if (*size > available) *size = available;
    if (*size) memcpy(stream->output + stream->output_pos, buffer, *size);
    stream->output_pos += *size;
}

static BOOL Mpq_PkwareDecode(const BYTE *src, DWORD src_size, BYTE *dst, DWORD dst_size, DWORD *out_size) {
    mpqPkwareStream_t stream = { src, src_size, 0, dst, dst_size, 0 };
    char *work = malloc(CMP_BUFFER_SIZE);
    unsigned int result;
    if (!work) return FALSE;
    result = explode(Mpq_PkwareRead, Mpq_PkwareWrite, work, &stream);
    free(work);
    if (result != 0 || stream.output_pos == 0) return FALSE;
    *out_size = stream.output_pos;
    return TRUE;
}
...
static BOOL Mpq_DecompressSector(mpqDecompress_t *p) {
    ...
    BYTE mask = p->src[0];
    if (mask == 0x08)
        return Mpq_PkwareDecode(p->src + 1, p->src_size - 1, p->dst, p->dst_size, &p->out_size);
...
static BOOL TryInflateSector(const BYTE *compressed, DWORD compressed_size, DWORD uncompressed_size, BYTE *out, DWORD *out_size)
{
    ...
    if (compressed_size > 1 && compressed[0] == 0x08) {
        if (!Mpq_PkwareDecode(compressed + 1, compressed_size - 1,
                              out, uncompressed_size, out_size)) {
            return FALSE;
        }
        return TRUE;
    }
