...
static BOOL FS_IsAbsolutePath(LPCSTR path) {
    if (!path || !path[0]) {
        return false;
    }
#ifdef _WIN32
...
bool FS_ResolveLoosePath(LPCSTR fileName, LPSTR out, DWORD out_size) {
    ...
    if (FS_IsAbsolutePath(fileName)) {
        if (!FS_FileOnDiskExists(fileName)) return false;
...
bool FS_FileExists(LPCSTR fileName) {
    if (FS_IsAbsolutePath(fileName)) {
        return FS_FileOnDiskExists(fileName);
    }
...
HANDLE FS_ReadLooseFile(LPCSTR filename, LPDWORD size, DWORD extraBytes) {
    if (FS_IsAbsolutePath(filename)) {
        ...
        fclose(file);
        if (extraBytes > 0) memset(buffer + fileSize, 0, extraBytes);
        if (size) *size = (DWORD)fileSize;
        return buffer;
    }
    FOR_LOOP(i, MAX_GAME_DIRS) {
        char path[MAX_PATHLEN * 2];
...
fsMapResolve_t FS_ResolveMapPath(LPCSTR name, LPSTR out, DWORD out_size) {
    ...
    if (FS_IsExplicitMapPath(name)) {
        if (FS_IsAbsolutePath(name)) {
            if (out && out_size > 0) {
                snprintf(out, out_size, "%s", name);
            }
            return FS_FileExists(name) ? FS_MAP_RESOLVE_OK : FS_MAP_RESOLVE_NOT_FOUND;
        }
